...
function inferLegacyJobPhase(job, progressPreview = []) {
  ...
  for (let index = progressPreview.length - 1; index >= 0; index -= 1) {
    ...
    if (line.startsWith("starting cursor") || line.startsWith("task ready") || line.startsWith("turn started")) {
      return "starting";
    }
...
export function resolveCancelableJob(cwd, reference) {
  ...
  if (activeJobs.length > 1) {
    throw new Error("Multiple Cursor Agent jobs are active. Pass a job id to /cursor:cancel.");
  }

  throw new Error("No active Cursor Agent jobs to cancel.");
}
