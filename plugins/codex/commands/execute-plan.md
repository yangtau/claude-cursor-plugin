---
description: Execute a Claude Code plan using Codex
tools: Read, Glob, Bash(node:*), Bash(mktemp:*), Write, AskUserQuestion
argument-hint: '[--write] [--model <model>] [--effort <none|minimal|low|medium|high|xhigh>] [plan-file-path]'
---

Execute a plan file using Codex.
...
2. Create a temporary prompt file by writing to `/tmp/codex-plan-<timestamp>.md` with the following content:
...
Foreground:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" task --write --prompt-file /tmp/codex-plan-<timestamp>.md
```

Background:
```bash
node "${CLAUDE_PLUGIN_ROOT}/scripts/codex-companion.mjs" task --background --write --prompt-file /tmp/codex-plan-<timestamp>.md
```

- Add `--model <model>` if the user specified one. If they ask for `spark`, map it to `gpt-5.3-codex-spark`.
- Add `--effort <level>` if the user specified one. Valid levels: none, minimal, low, medium, high, xhigh.
